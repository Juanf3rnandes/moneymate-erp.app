 ...
 */
export interface Action<T = unknown, M = T> {
  ...
  exec: (
    request: (() => ActionRequest<T>) | ActionRequest<T>,
    configs?: ActionConfig<T, M>
...
interface ActionConfig<T, M> {
  ...
  onError?: (error: HttpResponse<T>) => void;
 ...
 * @return Action
 */
export function useAction<T = unknown, M = T>(initialData?: M): Action<T, M> {
  ...
  async function exec(
    ...
    if (configs?.cacheKey && !configs?.cacheRefresh) {
      ...
      if (
        ...
        call = () => new Promise((resolve) => resolve(data as HttpResponse<T>));
        usingCache = true;
      }
    }
 ...
 */
export type Act<T = unknown, M = T> = Action<T, M> & {
 ...
 * @return Action
 */
export function useAct<T = unknown, M = T>(
  request: (() => ActionRequest<T>) | ActionRequest<T>,
  configs?: ActionConfig<T, M>,
  initialData?: M | undefined
): Act<T, M> {
  const action = useAction<T, M>(initialData);
